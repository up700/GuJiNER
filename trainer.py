...
logging_fh = logging.FileHandler('log.txt')
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.logging_steps == 0:
                        logger.info("  %s = %s", "train_loss", str(loss.item()))
                        results = self.evaluate("dev")
                        seq_f1 = results["seq_f1"]
                        if seq_f1 >= best_f1:
                            best_f1 = seq_f1
                            self.save_model() # 非 warndb 取消注释
                        logger.info("  %s = %s", "best_f1", str(best_f1))
                        log_dict = {"train_loss":loss.item(), "best_f1": best_f1, "seq_precision": results["seq_precision"], "seq_recall": results["seq_recall"], "seq_f1": results["seq_f1"]}
                        # wandb.log(log_dict)
