    ...
    with open(file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(line) != 3:
                ...
                w = line[0]
                label = line[-1] if line[-1] != "O" else line[-2]
            else:
                w = line[0]
                label = line[-1]

            if tmp == "O" and label != "O":
                sen += "{"
            if tmp != "O" and (label == "O" or tmp != label):
                sen += "|"+tmp+"}"
                if label != "O":
                    sen += "{"
...
def main():
    ...
    if args.do_predict:
        ...
        # output_test_predictions = "test_predictions.txt"
        predictions_save(test_file, predictions, output_test_predictions, labels)
        convert_format(output_test_predictions, args.max_span_len)
