...
print(knws)
fw1 = open("train/input.seq.char.1", "w", encoding="utf-8")
fw2 = open("train/output.seq.bio.1", "w", encoding="utf-8")

knw_to_doc = {}
with open("train/input.seq.char", "r", encoding="utf-8") as f1, open("train/output.seq.bio", "r", encoding="utf-8") as f2:
    for line1, line2 in zip(f1,f2):
        ind = knws.index("".join(line1.strip().split()))
        kn = data[ind]["source"]
        if kn not in knw_to_doc:
            knw_to_doc[kn] = []
        knw_to_doc[kn].append([line1.strip(), line2.strip()])

        # fw1.write(line1.strip()+" # "+" ".join(list(kn))+"\n")
        # fw2.write(line2.strip()+" # "+" ".join(list(kn))+"\n")

with open("train/knw_to_doc.json", "w", encoding="utf-8") as f:
    json.dump(knw_to_doc, f, indent=4, ensure_ascii=False)

max_length = 100
step = 5

for knw in knw_to_doc:
    docs = knw_to_doc[knw]
    doc_sum_char = []
    doc_sum_label = []
    for doc in docs:
        doc_sum_char += doc[0].split()
        doc_sum_label += doc[1].split()
    assert len(doc_sum_label) == len(doc_sum_char)

    l, r = 0, max_length
    while r <= len(doc_sum_char):
        fw1.write(" ".join(doc_sum_char[l:r])+"\n")
        fw2.write(" ".join(doc_sum_label[l:r])+"\n")
        l += step
        r = l + max_length
    if l < len(doc_sum_char):
        fw1.write(" ".join(doc_sum_char[l:])+"\n")
        fw2.write(" ".join(doc_sum_label[l:])+"\n")
